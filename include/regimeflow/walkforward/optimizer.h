...
namespace regimeflow::walkforward
{
    ...
    private:
        ...
        [[nodiscard]] std::vector<std::pair<TimeRange, TimeRange>> generate_windows(TimeRange full_range) const;
        ...
        [[nodiscard]] double compute_fitness(const engine::BacktestResults& results) const;
        [[nodiscard]] std::vector<ParameterSet> build_grid(const std::vector<ParameterDef>& params) const;
        ...
        engine::BacktestResults run_backtest(
            ...
            const std::function<std::unique_ptr<regime::RegimeDetector>()>& detector_factory,
            bool attach_detector) const;
        [[nodiscard]] std::map<regime::RegimeType, double> extract_regime_distribution(
            const engine::BacktestResults& results) const;
        [[nodiscard]] engine::BacktestResults stitch_oos_results(const std::vector<WindowResult>& windows) const;
