...
namespace regimeflow::risk
{
    ...
    public:
         ...
         */
        [[nodiscard]] virtual Result<void> validate(const engine::Order& order,
    ...
    public:
        ...
        [[nodiscard]] Result<void> validate(const engine::Order& order,
            ...
            if (order.limit_price <= 0 && order.type == engine::OrderType::Limit) {
                return Result<void>(Error(Error::Code::InvalidArgument, "Limit price must be set for notional checks"));
            }
            const double price = order.limit_price > 0 ? order.limit_price : 0.0;
            if (price <= 0) {
                return Ok();
            }
            const double order_notional = std::abs(order.quantity) * price;
            if (order_notional > max_notional_) {
                return Result<void>(Error(Error::Code::OutOfRange, "Order exceeds max notional limit"));
            }
            if (order_notional > portfolio.equity()) {
                return Result<void>(Error(Error::Code::OutOfRange, "Order exceeds portfolio equity"));
            }
            return Ok();
        }
    ...
    public:
        ...
        [[nodiscard]] Result<void> validate(const engine::Order& order,
            ...
            const Quantity existing = pos ? pos->quantity : 0;
            if (const Quantity projected = existing + order.quantity; std::abs(projected) > max_quantity_) {
                return Result<void>(Error(Error::Code::OutOfRange, "Order exceeds max position limit"));
            }
            return Ok();
        }
        ...
        [[nodiscard]] Result<void> validate_portfolio(const engine::Portfolio& portfolio) const override {
            for (const auto& position : portfolio.get_all_positions()) {
                if (std::abs(position.quantity) > max_quantity_) {
                    return Result<void>(Error(Error::Code::OutOfRange, "Position exceeds max position limit"));
                }
            }
            return Ok();
        }
    ...
    public:
        ...
        [[nodiscard]] Result<void> validate(const engine::Order& order,
            ...
            const double price = order.limit_price;
            if (price <= 0.0 && order.type == engine::OrderType::Limit) {
                return Result<void>(Error(Error::Code::InvalidArgument, "Limit price must be set for pct checks"));
            }
            if (price <= 0.0) {
                return Ok();
            }
            const auto pos = portfolio.get_position(order.symbol);
            const Quantity existing = pos ? pos->quantity : 0;
            const Quantity projected = existing + order.quantity;
            const double notional = std::abs(static_cast<double>(projected) * price);
            if (const double pct = notional / equity; pct > max_pct_) {
                return Result<void>(Error(Error::Code::OutOfRange, "Order exceeds max position pct limit"));
            }
            return Ok();
        }
        ...
        [[nodiscard]] Result<void> validate_portfolio(const engine::Portfolio& portfolio) const override {
            ...
            for (const auto& position : portfolio.get_all_positions()) {
                ...
                if (const double pct = notional / equity; pct > max_pct_) {
                    return Result<void>(Error(Error::Code::OutOfRange, "Position exceeds max position pct limit"));
                }
            }
            return Ok();
        }
    ...
    public:
        ...
        Result<void> validate(const engine::Order& order,
            ...
            if (const double drawdown = (peak_ - equity) / peak_; drawdown > max_drawdown_) {
                return Result<void>(Error(Error::Code::OutOfRange, "Max drawdown limit exceeded"));
            }
            return Ok();
        }
    ...
    public:
         ...
         */
        explicit MaxGrossExposureLimit(const double max_gross_exposure)
        ...
        [[nodiscard]] Result<void> validate(const engine::Order& order,
                              const engine::Portfolio& portfolio) const override {
            if (order.limit_price <= 0 && order.type == engine::OrderType::Limit) {
                return Result<void>(Error(Error::Code::InvalidArgument, "Limit price must be set for exposure checks"));
            }
            const double price = order.limit_price > 0 ? order.limit_price : 0.0;
            if (price <= 0) {
                return Ok();
            }
            if (const double projected = portfolio.gross_exposure() + std::abs(order.quantity) * price; projected > max_gross_exposure_) {
                return Result<void>(Error(Error::Code::OutOfRange, "Order exceeds max gross exposure limit"));
            }
            return Ok();
        }
        ...
        [[nodiscard]] Result<void> validate_portfolio(const engine::Portfolio& portfolio) const override {
            if (portfolio.gross_exposure() > max_gross_exposure_) {
                return Result<void>(Error(Error::Code::OutOfRange, "Max gross exposure limit exceeded"));
            }
            return Ok();
        }
    ...
    public:
        ...
        [[nodiscard]] Result<void> validate(const engine::Order& order,
                              const engine::Portfolio& portfolio) const override {
            if (order.limit_price <= 0 && order.type == engine::OrderType::Limit) {
                return Result<void>(Error(Error::Code::InvalidArgument, "Limit price must be set for leverage checks"));
            }
            const double price = order.limit_price > 0 ? order.limit_price : 0.0;
            if (price <= 0) {
                return Ok();
            ...
            if (const double leverage = projected / equity; leverage > max_leverage_) {
                return Result<void>(Error(Error::Code::OutOfRange, "Order exceeds max leverage limit"));
            }
            return Ok();
        }
        ...
        [[nodiscard]] Result<void> validate_portfolio(const engine::Portfolio& portfolio) const override {
            ...
            if (const double leverage = portfolio.gross_exposure() / equity; leverage > max_leverage_) {
                return Result<void>(Error(Error::Code::OutOfRange, "Max leverage limit exceeded"));
            }
            return Ok();
        }
    ...
    public:
         ...
         */
        explicit MaxNetExposureLimit(const double max_net_exposure)
        ...
        [[nodiscard]] Result<void> validate(const engine::Order& order,
            ...
            if (const double projected = portfolio.net_exposure() + (order.quantity * price); std::abs(projected) > max_net_exposure_) {
                return Result<void>(Error(Error::Code::OutOfRange, "Order exceeds max net exposure limit"));
            }
            return Ok();
        }
        ...
        [[nodiscard]] Result<void> validate_portfolio(const engine::Portfolio& portfolio) const override {
            if (std::abs(portfolio.net_exposure()) > max_net_exposure_) {
                return Result<void>(Error(Error::Code::OutOfRange, "Max net exposure limit exceeded"));
            }
            return Ok();
        }
    ...
    public:
        ...
        Result<void> validate(const engine::Order& order,
            ...
            const double price = order.limit_price;
            if (price <= 0.0 && order.type == engine::OrderType::Limit) {
                return Result<void>(Error(Error::Code::InvalidArgument, "Limit price must be set for sector checks"));
            }
            if (price <= 0.0) {
                return Ok();
            }
            const double sector_notional = sector_exposure(portfolio, sector);
            if (const double projected = sector_notional + std::abs(order.quantity) * price; projected / equity > limit) {
                return Result<void>(Error(Error::Code::OutOfRange, "Order exceeds sector exposure limit"));
            }
            return Ok();
        }
        ...
        Result<void> validate_portfolio(const engine::Portfolio& portfolio) const override {
            ...
            for (const auto& [sector, limit] : limits_) {
                if (const double exposure = sector_exposure(portfolio, sector); exposure / equity > limit) {
                    return Result<void>(Error(Error::Code::OutOfRange, "Sector exposure limit exceeded"));
                }
            }
            return Ok();
        }
    ...
    private:
        std::string sector_for(SymbolId symbol) const {
            ...
            const auto it = symbol_to_sector_.find(name);
        ...
        double sector_exposure(const engine::Portfolio& portfolio, const std::string& sector) const {
            ...
            for (const auto& pos : portfolio.get_all_positions()) {
                if (auto s = sector_for(pos.symbol); s == sector && pos.current_price > 0.0) {
    ...
    public:
        ...
        Result<void> validate(const engine::Order& order,
            ...
            const double price = order.limit_price;
            if (price <= 0.0 && order.type == engine::OrderType::Limit) {
                return Result<void>(Error(Error::Code::InvalidArgument, "Limit price must be set for industry checks"));
            }
            if (price <= 0.0) {
                return Ok();
            }
            if (const double notional = industry_exposure(portfolio, industry) + std::abs(order.quantity) * price; notional / equity > limit) {
                return Result<void>(Error(Error::Code::OutOfRange, "Order exceeds industry exposure limit"));
            }
            return Ok();
        }
        ...
        Result<void> validate_portfolio(const engine::Portfolio& portfolio) const override {
            ...
            for (const auto& [industry, limit] : limits_) {
                if (const double exposure = industry_exposure(portfolio, industry); exposure / equity > limit) {
                    return Result<void>(Error(Error::Code::OutOfRange, "Industry exposure limit exceeded"));
                }
            }
            return Ok();
        }
    ...
    private:
        std::string industry_for(const SymbolId symbol) const {
            const auto name = SymbolRegistry::instance().lookup(symbol);
            const auto it = symbol_to_industry_.find(name);
        ...
        double industry_exposure(const engine::Portfolio& portfolio,
            ...
            double exposure = 0.0;
            for (const auto& pos : portfolio.get_all_positions()) {
                if (auto s = industry_for(pos.symbol); s == industry && pos.current_price > 0.0) {
                    exposure += std::abs(pos.quantity * pos.current_price);
                }
            }
            return exposure;
    ...
    public:
         ...
         * @param symbol_to_sector Symbol -> sector mapping.
         */
        explicit MaxCorrelationExposureLimit(const Config& cfg,
                                             std::unordered_map<std::string, std::string> symbol_to_sector = {})
        ...
        Result<void> validate_portfolio(const engine::Portfolio& portfolio) const override {
            ...
            for (size_t i = 0; i < symbols.size(); ++i) {
                for (size_t j = i + 1; j < symbols.size(); ++j) {
                    if (const double corr = correlation(symbols[i], symbols[j]); std::abs(corr) < config_.max_corr) {
                        continue;
                    }
                    if (const double exposure = pair_exposure(portfolio, symbols[i], symbols[j]); exposure / equity > config_.max_pair_exposure_pct) {
                        return Result<void>(Error(Error::Code::OutOfRange, "Correlation exposure limit exceeded"));
                    }
                }
            }
            return Ok();
    ...
    private:
        ...
        double correlation(const SymbolId a, const SymbolId b) const {
            const auto itA = price_history_.find(a);
            const auto itB = price_history_.find(b);
        ...
        static double pair_exposure(const engine::Portfolio& portfolio, const SymbolId a, SymbolId b) {
            double exposure = 0.0;
            for (const auto& pos : portfolio.get_all_positions()) {
