...
namespace regimeflow::strategy
{
    ...
    private:
        ...
        [[nodiscard]] std::vector<Pivot> detect_pivots(const std::vector<double>& prices) const;
        [[nodiscard]] bool match_pattern(PatternType type,
                           double ab_xa,
                           double bc_ab,
                           double cd_bc,
                           double ad_xa,
                           double cd_xc) const;
        [[nodiscard]] double pattern_confidence(PatternType type,
                                  double ab_xa,
                                  double bc_ab,
                                  double cd_bc,
                                  double ad_xa,
                                  double cd_xc) const;
        [[nodiscard]] bool should_use_limit(const data::Bar& bar) const;
        [[nodiscard]] engine::Order build_order(engine::OrderSide side,
                                  const data::Bar& bar,
                                  PatternType pattern,
