        ...
        for (const auto& [key, prop] : schema.properties) {
            ...
            if (!value) {
                if (prop.required && !prop.default_value.has_value()) {
                    return Result<void>(Error(Error::Code::ConfigError, "Missing required config field: " + key));
                }
                continue;
            }
            if (!prop.type.empty() && !config_value_matches(*value, prop.type)) {
                return Result<void>(Error(Error::Code::ConfigError, "Config field type mismatch: " + key));
            }
        }
        return Ok();
    }
