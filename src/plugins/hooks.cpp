...
{
    void HookManager::register_hook(const HookType type, Hook hook, const int priority) {
        auto& list = hooks_for(type);
        list.push_back(Entry{std::move(hook), priority, next_sequence_++});
        sort_hooks(list);
    }

    void HookManager::register_backtest_start(BacktestStartHook hook, const int priority) {
        register_hook(HookType::BacktestStart, std::move(hook), priority);
    }

    void HookManager::register_backtest_end(BacktestEndHook hook, const int priority) {
    ...
    void HookManager::register_day_start(DayStartHook hook, const int priority) {
    ...
    void HookManager::register_day_end(DayEndHook hook, const int priority) {
    ...
    void HookManager::register_on_bar(BarHook hook, const int priority) {
    ...
    void HookManager::register_on_tick(TickHook hook, const int priority) {
    ...
    void HookManager::register_on_quote(QuoteHook hook, const int priority) {
    ...
    void HookManager::register_on_book(BookHook hook, const int priority) {
    ...
    void HookManager::register_on_timer(TimerHook hook, const int priority) {
    ...
    void HookManager::register_order_submit(OrderSubmitHook hook, const int priority) {
    ...
    void HookManager::register_on_fill(FillHook hook, const int priority) {
    ...
    void HookManager::register_regime_change(RegimeChangeHook hook, const int priority) {
    ...
    HookResult HookManager::invoke(const HookType type, HookContext& ctx) const {
        if (!hooks_enabled_) {
            return HookResult::Continue;
        }
        for (const auto& list = hooks_for(type); const auto& entry : list) {
            const auto result = entry.hook(ctx);
    ...
    std::vector<HookManager::Entry>& HookManager::hooks_for(const HookType type) {
        return hooks_[type];
    }
