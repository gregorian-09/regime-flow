...
namespace regimeflow::plugins
{
    ...
    PluginRegistry::PluginPtr PluginRegistry::create_plugin(const std::string& type,
        ...
        std::lock_guard<std::mutex> lock(mutex_);
        const auto type_it = factories_.find(type);
        if (type_it == factories_.end()) {
            return nullptr;
        }
        const auto plugin_it = type_it->second.find(name);
        ...
        if (const auto res = plugin->on_load(); res.is_err()) {
            return nullptr;
        }
    ...
    std::vector<std::string> PluginRegistry::list_types() const {
        ...
        for (const auto& type : factories_ | std::views::keys) {
    ...
    std::vector<std::string> PluginRegistry::list_plugins(const std::string& type) const {
        ...
        for (const auto& name : it->second | std::views::keys) {
    ...
    std::optional<PluginInfo> PluginRegistry::get_info(const std::string& type,
                                                       const std::string& name) const {
        const auto plugin = create_plugin(type, name);
        if (!plugin) {
            return std::nullopt;
        }
        return plugin->info();
    ...
    Result<void> PluginRegistry::load_dynamic_plugin(const std::string& path) {
        ...
        if (auto abi_fn = reinterpret_cast<StrFn>(dlsym(handle, "regimeflow_abi_version"))) {
            if (const char* version = abi_fn(); !version || std::string(version) != REGIMEFLOW_ABI_VERSION) {
                dlclose(handle);
                return Result<void>(Error(Error::Code::InvalidState, "Plugin ABI version mismatch"));
            }
        }
        ...
        {
            std::lock_guard<std::mutex> lock(mutex_);
            if (dynamic_plugins_.contains(plugin_name)) {
                destroy_fn(probe.release());
                dlclose(handle);
                return Result<void>(Error(Error::Code::AlreadyExists, "Plugin already loaded"));
            }
    ...
    Result<void> PluginRegistry::unload_dynamic_plugin(const std::string& name) {
        ...
        {
            std::lock_guard<std::mutex> lock(mutex_);
            const auto it = dynamic_plugins_.find(name);
            if (it == dynamic_plugins_.end()) {
                return Result<void>(Error(Error::Code::NotFound, "Plugin not loaded"));
            }
            plugin = it->second;
            dynamic_plugins_.erase(it);
            if (const auto type_it = factories_.find(plugin.type); type_it != factories_.end()) {
                type_it->second.erase(name);
                if (type_it->second.empty()) {
                    factories_.erase(type_it);
                }
