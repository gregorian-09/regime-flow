...
namespace regimeflow::strategy
{
    ...
    Result<engine::OrderId> StrategyContext::submit_order(engine::Order order) const
    {
        if (!order_manager_) {
            return Result<engine::OrderId>(Error(Error::Code::InvalidState, "Order manager not available"));
        }
        if (order.created_at.microseconds() == 0) {
            order.created_at = current_time();
            order.updated_at = order.created_at;
        }
        switch (current_regime().regime) {
    ...
    Result<void> StrategyContext::cancel_order(engine::OrderId id) const
    {
        if (!order_manager_) {
            return Result<void>(Error(Error::Code::InvalidState, "Order manager not available"));
        }
    ...
    std::optional<data::Bar> StrategyContext::latest_bar(const SymbolId symbol) const {
        return market_data_ ? market_data_->latest_bar(symbol) : std::nullopt;
    }

    std::optional<data::Tick> StrategyContext::latest_tick(const SymbolId symbol) const {
        return market_data_ ? market_data_->latest_tick(symbol) : std::nullopt;
    }

    std::optional<data::Quote> StrategyContext::latest_quote(const SymbolId symbol) const {
        return market_data_ ? market_data_->latest_quote(symbol) : std::nullopt;
    }

    std::vector<data::Bar> StrategyContext::recent_bars(const SymbolId symbol, const size_t count) const {
        return market_data_ ? market_data_->recent_bars(symbol, count) : std::vector<data::Bar>{};
    }

    std::optional<data::OrderBook> StrategyContext::latest_order_book(const SymbolId symbol) const {
    ...
    void StrategyContext::schedule_timer(const std::string& id, const Duration interval) const
    {
        if (!timer_service_) {
            return;
        }
    ...
    void StrategyContext::cancel_timer(const std::string& id) const
    {
        if (!timer_service_) {
            return;
        }
