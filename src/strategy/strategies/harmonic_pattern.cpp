...
namespace regimeflow::strategy
{
    namespace {
        ...
        double ratio(const double num, const double den) {
            if (den == 0.0) {
                return 0.0;
            }
        ...
        bool in_range(const double value, const double min_v, const double max_v, const double tol) {
            const double low = min_v * (1.0 - tol);
            const double high = max_v * (1.0 + tol);
            return value >= low && value <= high;
        }

        double score_ratio(const double value, const double min_v, const double max_v, const double tol) {
            if (!in_range(value, min_v, max_v, tol)) {
                return 0.0;
            }
            const double target = 0.5 * (min_v + max_v);
            const double span = std::abs(max_v - min_v);
            const double max_diff = span > 0.0 ? span * 0.5 : std::max(1e-6, std::abs(target) * tol);
            const double diff = std::abs(value - target);
            const double score = 1.0 - std::min(diff / max_diff, 1.0);
    ...
    bool HarmonicPatternStrategy::load_symbol_from_config() {
        ...
        if (const auto v = ctx_->config().get_as<std::string>("symbol")) {
            symbol_ = *v;
        }
        if (!symbol_.empty()) {
    ...
    HarmonicPatternStrategy::detect_pivots(const std::vector<double>& prices) const {
        ...
        for (size_t i = 1; i < prices.size(); ++i) {
            ...
            const double change = (price - last_price) / (last_price == 0.0 ? 1.0 : last_price);
            if (trend == 0) {
    ...
    bool HarmonicPatternStrategy::match_pattern(const PatternType type,
                                                const double ab_xa,
                                                const double bc_ab,
                                                const double cd_bc,
                                                const double ad_xa,
                                                const double cd_xc) const {
        switch (type) {
    ...
    double HarmonicPatternStrategy::pattern_confidence(const PatternType type,
                                                       const double ab_xa,
                                                       const double bc_ab,
                                                       const double cd_bc,
                                                       const double ad_xa,
    ...
    bool HarmonicPatternStrategy::should_use_limit(const data::Bar& bar) const {
        ...
        const double vol = range / bar.close;
        return vol >= vol_threshold_pct_;
    }

    engine::Order HarmonicPatternStrategy::build_order(const engine::OrderSide side,
                                                       const data::Bar& bar,
                                                       const PatternType pattern,
                                                       const double confidence) const {
        const bool use_limit = should_use_limit(bar);
        ...
        if (passive_weight + aggressive_weight > 1.0) {
            const double scale = 1.0 / (passive_weight + aggressive_weight);
            passive_weight *= scale;
            aggressive_weight *= scale;
        }
        const double lit_weight = 1.0 - passive_weight - aggressive_weight;
        const std::string weight_tag = "p:" + std::to_string(passive_weight)
            + "|a:" + std::to_string(aggressive_weight)
            + "|l:" + std::to_string(lit_weight);
        if (use_limit && !aggressive) {
            const double offset = limit_offset_bps_ / 10'000.0;
    ...
    void HarmonicPatternStrategy::on_bar(const data::Bar& bar) {
        ...
        constexpr PatternType types[] = {PatternType::Gartley, PatternType::Bat, PatternType::Butterfly,
        ...
        auto [type, confidence] = *std::ranges::max_element(candidates,
