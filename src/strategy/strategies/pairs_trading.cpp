...
namespace regimeflow::strategy
{
    namespace {
        ...
        double stddev(const std::vector<double>& v, const double m) {
            if (v.size() < 2) {
                return 0.0;
            }
            double s = 0.0;
    ...
    void PairsTradingStrategy::initialize(StrategyContext& ctx) {
        ...
        if (const auto v = ctx.config().get_as<int64_t>("lookback")) {
            lookback_ = static_cast<size_t>(std::max<int64_t>(30, *v));
        }
        if (const auto v = ctx.config().get_as<double>("entry_z")) {
            entry_z_ = std::max(0.5, *v);
        }
        if (const auto v = ctx.config().get_as<double>("exit_z")) {
            exit_z_ = std::max(0.1, *v);
        }
        if (const auto v = ctx.config().get_as<double>("max_z")) {
            max_z_ = std::max(entry_z_, *v);
        }
        if (const auto v = ctx.config().get_as<bool>("allow_short")) {
            allow_short_ = *v;
        }
        if (const auto v = ctx.config().get_as<int64_t>("base_qty")) {
            base_qty_ = static_cast<Quantity>(std::max<int64_t>(1, *v));
        }
        if (const auto v = ctx.config().get_as<double>("min_qty_scale")) {
            min_qty_scale_ = std::max(0.1, *v);
        }
        if (const auto v = ctx.config().get_as<double>("max_qty_scale")) {
            max_qty_scale_ = std::max(min_qty_scale_, *v);
        }
        if (const auto v = ctx.config().get_as<int64_t>("cooldown_bars")) {
    ...
    bool PairsTradingStrategy::load_symbols_from_config() {
        ...
        if (const auto v = ctx_->config().get_as<std::string>("symbol_a")) {
            symbol_a_ = *v;
        }
        if (const auto v = ctx_->config().get_as<std::string>("symbol_b")) {
            symbol_b_ = *v;
        }
        if (!symbol_a_.empty()) {
    ...
    bool PairsTradingStrategy::compute_spread(double& spread, double& hedge_ratio) const {
        ...
        const auto bars_a = ctx_->recent_bars(symbol_a_id_, lookback_);
        const auto bars_b = ctx_->recent_bars(symbol_b_id_, lookback_);
        if (bars_a.size() < lookback_ || bars_b.size() < lookback_) {
            return false;
        }
        const size_t n = std::min(bars_a.size(), bars_b.size());
        ...
        const double mean_a = mean(a_prices);
        const double mean_b = mean(b_prices);
        double cov = 0.0;
        double var_b = 0.0;
        for (size_t i = 0; i < n; ++i) {
    ...
    Quantity PairsTradingStrategy::scaled_qty(const double zscore) const {
        const double abs_z = std::min(std::abs(zscore), max_z_);
        double scale = min_qty_scale_;
        if (max_z_ > entry_z_) {
            const double t = (abs_z - entry_z_) / (max_z_ - entry_z_);
    ...
    void PairsTradingStrategy::submit_spread_trade(const double hedge_ratio,
                                                   const double zscore,
                                                   ...
                                                   const double price_b) const
    {
        ...
        const Quantity qty = scaled_qty(zscore);
        const Quantity qty_b = static_cast<Quantity>(std::max(1.0, std::round(qty * std::abs(hedge_ratio))));

        const engine::OrderSide side_a = zscore > 0 ? engine::OrderSide::Sell : engine::OrderSide::Buy;
        const engine::OrderSide side_b = zscore > 0 ? engine::OrderSide::Buy : engine::OrderSide::Sell;
