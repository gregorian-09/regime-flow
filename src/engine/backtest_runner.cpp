...
namespace regimeflow::engine
{
    ...
    BacktestResults BacktestRunner::run(std::unique_ptr<strategy::Strategy> strategy,
                                        ...
                                        const data::BarType bar_type) const
    {
    ...
    namespace {
        ...
        BacktestResults run_spec(const BacktestRunSpec& spec) {
            const auto engine = EngineFactory::create(spec.engine_config);
            if (!engine) {
                throw std::runtime_error("Failed to create backtest engine");
            }
            const auto data_source = data::DataSourceFactory::create(spec.data_config);
            if (!data_source) {
            ...
            const BacktestRunner runner(engine.get(), data_source.get());
