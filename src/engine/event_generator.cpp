...
namespace regimeflow::engine
{
    ...
    void EventGenerator::enqueue_all() const
    {
        ...
        std::ranges::sort(events, [](const events::Event& a, const events::Event& b) {
            ...
            const auto a_payload = std::get_if<events::MarketEventPayload>(&a.payload);
            if (auto b_payload = std::get_if<events::MarketEventPayload>(&b.payload); a_payload && b_payload && a_payload->kind != b_payload->kind) {
        ...
        if (!events.empty()) {
            ...
            for (const auto& evt : events) {
                if (auto day = evt.timestamp.to_string("%Y%m%d"); first_day || day != current_day) {
            ...
            if (config_.emit_regime_check &&
                ...
                const Timestamp end = events.back().timestamp;
                while (cursor <= end) {
                    system_events.push_back(events::make_system_event(
            ...
            if (!system_events.empty()) {
                ...
                std::ranges::sort(events, [](const events::Event& a,
                    ...
                    const auto a_payload = std::get_if<events::MarketEventPayload>(&a.payload);
                    if (const auto b_payload = std::get_if<events::MarketEventPayload>(&b.payload); a_payload && b_payload && a_payload->kind != b_payload->kind) {
                        return a_payload->kind < b_payload->kind;
                    }
                    return false;
                });
