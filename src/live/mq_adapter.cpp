...
namespace regimeflow::live
{
    ...
    public:
        ...
        Result<void> publish(const LiveMessage& message) override {
            if (!connected_) {
                return Result<void>(Error(Error::Code::InvalidState, "ZeroMQ adapter not connected"));
            }
            std::string topic = config_.topic.empty() ? "regimeflow" : config_.topic;
    ...
    public:
        ...
        Result<void> connect() override {
            ...
            if (!pub_ctx_ || pub_ctx_->err) {
                return Result<void>(Error(Error::Code::InvalidState, "Redis publish connection failed"));
            }
            ...
            if (!sub_ctx_ || sub_ctx_->err) {
                return Result<void>(Error(Error::Code::InvalidState, "Redis subscribe connection failed"));
            }
        ...
        Result<void> publish(const LiveMessage& message) override {
            if (!connected_) {
                return Result<void>(Error(Error::Code::InvalidState, "Redis adapter not connected"));
            }
            const std::string payload = LiveMessageCodec::encode(message);
            auto* reply = static_cast<redisReply*>(
            ...
            if (reply->type == REDIS_REPLY_ARRAY) {
                for (size_t i = 0; i < reply->elements; ++i) {
                    ...
                    for (size_t j = 0; j < entries->elements; ++j) {
                        ...
                        auto* ack = static_cast<redisReply*>(
    ...
    public:
        ...
        Result<void> connect() override {
            ...
            if (!producer_) {
                return Result<void>(Error(Error::Code::InvalidState, errstr));
            }
        ...
        Result<void> publish(const LiveMessage& message) override {
            if (!connected_ || !producer_) {
                return Result<void>(Error(Error::Code::InvalidState, "Kafka adapter not connected"));
            }
            std::string payload = LiveMessageCodec::encode(message);
            std::string topic = config_.topic.empty() ? "regimeflow" : config_.topic;
...
#pragma clang diagnostic pop
            ...
            if (res != 0) {
                connected_ = false;
                return Result<void>(Error(Error::Code::InvalidState, rd_kafka_err2str(rd_kafka_last_error())));
            }
