...
namespace regimeflow::risk
{
    Quantity FixedFractionalSizer::size(const PositionSizingContext& ctx) const {
        ...
        const double risk_capital = ctx.equity * risk_per_trade_;
        const double stop_distance = std::abs(ctx.price - ctx.stop_price);
        if (stop_distance <= 0.0) {
            return 0;
        }
        const double qty = risk_capital / stop_distance;
    ...
    Quantity VolatilityTargetSizer::size(const PositionSizingContext& ctx) const {
        ...
        const double qty = (target_vol_ * ctx.equity) / (ctx.volatility * ctx.price);
        return static_cast<Quantity>(qty);
    }
    ...
    Quantity KellySizer::size(const PositionSizingContext& ctx) const {
        ...
        const double win = ctx.win_rate;
        const double loss = 1.0 - win;
        ...
        const double qty = (kelly * ctx.equity) / ctx.price;
        return static_cast<Quantity>(qty);
    }
