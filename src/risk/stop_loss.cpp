...
namespace regimeflow::risk
{
    ...
    void StopLossManager::update_atr(const SymbolId symbol, const data::Bar& bar) {
    ...
    void StopLossManager::update_from_price(const SymbolId symbol, const Price price, const Timestamp ts,
    ...
    void StopLossManager::maybe_exit(const SymbolId symbol, const Price price, const Timestamp ts,
                                     engine::OrderManager& order_manager) {
        const auto it = states_.find(symbol);
        if (it == states_.end()) {
            return;
        ...
        const bool is_long = state.last_qty > 0;
        bool trigger = false;
