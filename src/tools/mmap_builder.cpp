...
namespace regimeflow::data
{
    namespace {
        ...
        std::string bar_type_suffix(const BarType type) {
            switch (type) {
        ...
        TimeRange parse_range(const Args& args, const DataSource& source, const SymbolId symbol) {
            TimeRange range;
            if (!args.start.empty()) {
            ...
            if (range.start.microseconds() == 0 || range.end.microseconds() == 0) {
                auto [start, end] = source.get_available_range(symbol);
        ...
        BarBuilder::Config bar_builder_config(const BarType bar_type, const Args& args) {
