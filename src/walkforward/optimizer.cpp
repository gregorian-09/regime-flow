...
namespace
{
    ...
    double apply_step(const double value, const double min_v, const double max_v, const double step) {
        if (step <= 0.0) {
            return clamp_value(value, min_v, max_v);
        }
        const double steps = std::round((value - min_v) / step);
    ...
    std::variant<int, double, std::string> sample_param_value(
        ...
        case regimeflow::walkforward::ParameterDef::Distribution::LogUniform: {
                const double safe_min = std::max(min_v, 1e-12);
        ...
        case regimeflow::walkforward::ParameterDef::Distribution::Normal: {
                const double mean = 0.5 * (min_v + max_v);
                if (const double stddev = (max_v - min_v) / 6.0; stddev <= 0.0) {
                    value = mean;
                } else {
...
namespace regimeflow::walkforward
{
    ...
    WalkForwardResults WalkForwardOptimizer::optimize(
        ...
        std::function<std::unique_ptr<regime::RegimeDetector>()> detector_factory) const
    {
        ...
        for (const auto& [is_range, oos_range] : windows) {
            ...
            if (config_.optimization_method == WalkForwardConfig::OptMethod::Bayesian) {
                ...
                for (int i = 0; i < total_trials; ++i) {
                    ...
                    if (i < warmup || best_params.empty()) {
                        ...
                        for (const auto& param : params) {
                            if (param.type == ParameterDef::Type::Categorical) {
                                if (std::uniform_real_distribution<double> pick(0.0, 1.0); pick(rng) < 0.7) {
                                    if (auto it = best_params.find(param.name); it != best_params.end()) {
                    ...
                    if (config_.maximize ? (fitness > best_fitness) : (fitness < best_fitness)) {
                ...
                for (size_t i = 0; i < outcomes.size(); ++i) {
                    ...
                    if (bool better = config_.maximize ? (fitness > best_fitness) : (fitness < best_fitness); better || (fitness == best_fitness && i < best_index)) {
                        best_fitness = fitness;
                        best_index = i;
                    }
                }
    ...
    std::vector<std::pair<TimeRange, TimeRange>> WalkForwardOptimizer::generate_windows(
        ...
        const Timestamp end = full_range.end;
    ...
    std::vector<std::pair<TimeRange, TimeRange>> WalkForwardOptimizer::generate_regime_windows(
        ...
        std::vector<std::pair<TimeRange, TimeRange>> windows;
        for (auto base_windows = generate_windows(full_range); const auto& [base_is, base_oos] : base_windows) {
    ...
    double WalkForwardOptimizer::compute_fitness(const engine::BacktestResults& results) const {
        if (config_.optimize_per_regime || config_.window_type == WalkForwardConfig::WindowType::RegimeAware) {
            if (const auto& regimes = results.metrics.regime_attribution().results(); !regimes.empty()) {
                double weighted = 0.0;
                double weight_sum = 0.0;
                for (const auto& perf : regimes | std::views::values) {
                    double metric = 0.0;
        ...
        const auto stats = metrics::compute_stats(results.metrics.equity_curve(), config_.periods_per_year);
    ...
    std::vector<ParameterSet> WalkForwardOptimizer::build_grid(
        ...
        for (const auto& param : params) {
            ...
            if (param.type == ParameterDef::Type::Categorical) {
                ...
                const double step = param.step != 0 ? param.step : 1.0;
                for (const auto& base : grid) {
                    for (int idx = 0;; ++idx) {
    ...
    std::vector<WalkForwardOptimizer::TrialOutcome> WalkForwardOptimizer::evaluate_trials(
        ...
        const bool attach_detector) const {
        ...
        auto worker = [&]() {
            while (true) {
                ...
                try {
                    ...
                    const double fitness = compute_fitness(results);
    ...
    engine::BacktestResults WalkForwardOptimizer::run_backtest(
        ...
        const std::function<std::unique_ptr<strategy::Strategy>(const ParameterSet&)>& factory,
        data::DataSource* data,
        const TimeRange& range,
        ...
        if (attach_detector && detector_factory) {
            ...
            if (detector && config_.retrain_regime_each_window) {
                ...
                if (!handled && !config_.disable_default_regime_training) {
                    ...
                    if (!symbols.empty()) {
                        if (const auto iterator = data->create_iterator(symbols, training_range, config_.bar_type))
                        {
                            regime::FeatureExtractor extractor;
                            while (iterator->has_next()) {
                                auto bar = iterator->next();
                                if (auto feature = extractor.on_bar(bar); !feature.empty()) {
        ...
        const engine::BacktestRunner runner(&engine, data);
    ...
    std::map<regime::RegimeType, double> WalkForwardOptimizer::extract_regime_distribution(
        ...
        for (const auto& regimes = results.metrics.regime_attribution().results(); const auto& [regime, perf] : regimes) {
    ...
    engine::BacktestResults WalkForwardOptimizer::stitch_oos_results(
        ...
        for (const auto& window : windows) {
            if (const auto& curve = window.oos_results.metrics.equity_curve(); !initialized && !curve.equities().empty()) {
    ...
    void WalkForwardOptimizer::analyze_oos_performance(WalkForwardResults& results) const {
        ...
        for (const auto& window : results.windows) {
            for (const auto& regimes = window.oos_results.metrics.regime_attribution().results(); const auto& [regime, perf] : regimes) {
        ...
        for (const auto& [regime, total] : weighted_sum) {
            const double denom = weight_sum[regime];
        ...
        if (regime_values.size() <= 1) {
            ...
            double mean = 0.0;
            for (const double v : regime_values) {
                mean += v;
            }
