...
namespace regimeflow
{
    ...
    Timestamp Timestamp::from_string(const std::string& str, const std::string& fmt) {
        ...
        const time_t seconds = timegm_utc(&tm);
    ...
    Timestamp Timestamp::from_date(const int year, const int month, const int day) {
        std::tm tm = {};
    ...
    std::string Timestamp::to_string(const std::string& fmt) const {
        auto seconds = static_cast<std::time_t>(us_ / 1'000'000);
        std::tm tm = {};
#if defined(_WIN32)
        gmtime_s(&tm, &seconds);
#else
