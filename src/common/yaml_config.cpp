    ...
    namespace {
        ...
        std::string unescape(const std::string& value) {
            ...
            for (size_t i = 0; i < value.size(); ++i) {
                if (const char c = value[i]; c == '\\' && i + 1 < value.size()) {
                    switch (const char n = value[++i]) {
        ...
        ConfigValue::Array parse_inline_array(const std::string& raw) {
            const std::string content = trim(raw.substr(1, raw.size() - 2));
        ...
        ConfigValue parse_scalar(const std::string& raw) {
            ...
            char* end = nullptr;
            const long long int_val = std::strtoll(value.c_str(), &end, 10);
            if (end && *end == '\0') {
    ...
    Config YamlConfigLoader::load_file(const std::string& path) {
        ...
        while (std::getline(file, line)) {
            ...
            if (starts_with(trimmed, "- ")) {
                ...
                if (!key_stack.empty()) {
                    const std::string& path_key = key_stack.back();
            ...
            if (current_array_item_object && !current_array_path.empty() && indent > current_array_indent) {
                if (auto existing = config.get_path(current_array_path); existing && existing->get_if<ConfigValue::Array>()) {
                    if (auto array = *existing->get_if<ConfigValue::Array>(); !array.empty()) {
                        ...
                        if (!current_array_object_key.empty() && indent > current_array_object_indent) {
                            if (auto nested_it = obj->find(current_array_object_key); nested_it == obj->end() ||
