...
namespace regimeflow::data
{
    namespace {
        ...
        void ensure_required_columns(const std::map<std::string, int>& mapping) {
            for (const char* required[] = {"timestamp", "price", "quantity"}; const auto* name : required) {
    ...
    TimeRange CSVTickDataSource::get_available_range(const SymbolId symbol) const {
        TimeRange range;
        const std::string path = resolve_path(symbol);
        if (path.empty()) {
            return range;
        }
        if (auto ticks = parse_ticks(symbol, path, range); !ticks.empty()) {
    ...
    std::vector<Tick> CSVTickDataSource::get_ticks(const SymbolId symbol, const TimeRange range) {
        const std::string path = resolve_path(symbol);
        if (path.empty()) {
            return {};
        }
    ...
    std::unique_ptr<DataIterator> CSVTickDataSource::create_iterator(
        const std::vector<SymbolId>& symbols, const TimeRange range, BarType) {
        std::vector<Bar> bars;
    ...
    std::unique_ptr<TickIterator> CSVTickDataSource::create_tick_iterator(
        const std::vector<SymbolId>& symbols, const TimeRange range) {
    ...
    std::string CSVTickDataSource::resolve_path(SymbolId symbol) const {
        if (auto it = symbol_to_path_.find(symbol); it != symbol_to_path_.end()) {
            return it->second;
        }
    ...
    std::vector<Tick> CSVTickDataSource::parse_ticks(SymbolId symbol, const std::string& path,
        ...
        int price_col = col.contains("price") ? col["price"] : 1;
        int qty_col = col.contains("quantity") ? col["quantity"] : 2;
        ...
        while (std::getline(file, line)) {
            ...
            if (config_.validation.check_gap && has_last_ts) {
                if (Duration gap = tick.timestamp - last_ts; gap.total_microseconds() > config_.validation.max_gap.total_microseconds()) {
            ...
            if (config_.validation.check_price_jump && has_last_price && last_price != 0.0) {
                if (double jump = std::abs(tick.price - last_price) / std::abs(last_price); jump > config_.validation.max_jump_pct) {
