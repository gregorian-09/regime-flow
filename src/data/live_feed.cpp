...
namespace regimeflow::data
{
    ...
    Result<void> PollingRestFeed::connect() {
        if (!config_.source) {
            return Result<void>(Error(Error::Code::InvalidState, "Live feed source not configured"));
        }
        connected_ = true;
        return Ok();
    }
    ...
    void PollingRestFeed::poll() {
        ...
        for (auto symbol : subscribed_) {
            ...
            for (auto bars = config_.source->get_bars(symbol, range, config_.bar_type); const auto& bar : bars) {
            ...
            for (auto ticks = config_.source->get_ticks(symbol, range); const auto& tick : ticks) {
