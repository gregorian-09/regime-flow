...
namespace regimeflow::data
{
    namespace {
        ...
        bool handle_issue(const ValidationSeverity severity,
                          const ValidationAction action,
                          const std::string& message,
                          const bool collect_report,
        ...
        ValidationAction normalize_error_action(const ValidationAction action) {
    ...
    std::optional<Duration> bar_interval_for(const BarType bar_type) {
        switch (bar_type) {
    ...
    std::vector<Bar> fill_missing_time_bars(const std::vector<Bar>& bars, const Duration interval) {
        if (bars.size() < 2) {
            return bars;
        }
        ...
        for (size_t i = 0; i < bars.size(); ++i) {
            ...
            if (config.check_price_bounds) {
                if (!std::isfinite(bar.open) || !std::isfinite(bar.high) ||
                    ...
                    if (const auto action = normalize_error_action(config.on_error); !handle_issue(ValidationSeverity::Error, action, "OHLC out of range",
            ...
            if (config.check_volume_bounds && config.max_volume > 0 && bar.volume > config.max_volume) {
                if (const auto action = normalize_error_action(config.on_error); !handle_issue(ValidationSeverity::Error, action, "Volume exceeds max_volume",
                                                                                         collect_report, report, i + 1)) {
                    continue;
                                                                                         }
            }
            ...
            if (config.check_future_timestamps) {
                if (bar.timestamp > now + config.max_future_skew) {
                    if (const auto action = normalize_error_action(config.on_error); !handle_issue(ValidationSeverity::Error, action, "Timestamp is in the future",
                                                                                             collect_report, report, i + 1)) {
                        continue;
                                                                                             }
                }
            ...
            if (config.check_trading_hours &&
                ...
                if (const auto action = normalize_error_action(config.on_error); !handle_issue(ValidationSeverity::Error, action, "Timestamp outside trading hours",
                                                                                         collect_report, report, i + 1)) {
                    continue;
                                                                                         }
                                      }
            ...
            if (config.require_monotonic_timestamps && has_last_ts && bar.timestamp < last_ts) {
                if (const auto action = normalize_error_action(config.on_error); !handle_issue(ValidationSeverity::Error, action, "Non-monotonic timestamp",
                                                                                         collect_report, report, i + 1)) {
                    continue;
                                                                                         }
            }
            ...
            if (config.check_gap && has_last_ts) {
                if (Duration gap = bar.timestamp - last_ts; gap.total_microseconds() > config.max_gap.total_microseconds()) {
                    const auto action = config.on_gap;
            ...
            if (config.check_price_jump && has_last_close && last_close != 0.0) {
                if (const double jump = std::abs(bar.close - last_close) / std::abs(last_close); jump > config.max_jump_pct) {
            ...
            if (config.check_outliers && price_stats.count >= config.outlier_warmup) {
                if (const double stddev = price_stats.stddev(); stddev > 0.0) {
            ...
            if (config.check_outliers && volume_stats.count >= config.outlier_warmup) {
                if (const double stddev = volume_stats.stddev(); stddev > 0.0) {
        ...
        for (size_t i = 0; i < ticks.size(); ++i) {
            ...
            if (config.check_price_bounds) {
                if (!std::isfinite(tick.price) || tick.price <= 0 ||
                    ...
                    if (const auto action = normalize_error_action(config.on_error); !handle_issue(ValidationSeverity::Error, action, "Invalid price",
                                                                                             collect_report, report, i + 1)) {
                        continue;
                                                                                             }
                    }
            ...
            if (config.check_volume_bounds && config.max_volume > 0 &&
                ...
                const auto action = normalize_error_action(config.on_error);
            ...
            if (config.check_gap && has_last_ts) {
                if (Duration gap = tick.timestamp - last_ts; gap.total_microseconds() > config.max_gap.total_microseconds()) {
            ...
            if (config.check_price_jump && has_last_price && last_price != 0.0) {
                if (const double jump = std::abs(tick.price - last_price) / std::abs(last_price); jump > config.max_jump_pct) {
                    if (!handle_issue(ValidationSeverity::Warning, config.on_warning,
                                      "Price jump exceeds max_jump_pct", collect_report, report,
                                      i + 1)) {
                        continue;
            ...
            if (config.check_outliers && price_stats.count >= config.outlier_warmup) {
                const double stddev = price_stats.stddev();
                if (stddev > 0.0) {
            ...
            if (config.check_outliers && volume_stats.count >= config.outlier_warmup) {
                const double stddev = volume_stats.stddev();
                if (stddev > 0.0) {
