...
namespace regimeflow::data
{
    namespace {
        ...
        bool checked_mul(const size_t a, const size_t b, size_t& out) {
            if (a == 0 || b == 0) {
                out = 0;
                return true;
            }
        ...
        bool checked_add(const size_t a, const size_t b, size_t& out) {
            if (a > std::numeric_limits<size_t>::max() - b) {
                return false;
            }
            out = a + b;
        ...
        void write_bytes(std::ofstream& out, const void* data, const size_t len, Sha256* sha) {
    ...
    OrderBook OrderBookMmapFile::at(size_t index) const {
        ...
        for (size_t level = 0; level < kLevels; ++level) {
            const size_t offset = level * header_->book_count + index;
    ...
    std::pair<size_t, size_t> OrderBookMmapFile::find_range(TimeRange range) const {
        ...
        const int64_t start = range.start.microseconds();
        const int64_t end = range.end.microseconds();
        const auto begin_it = std::lower_bound(timestamps_, timestamps_ + count, start);
        const auto end_it = std::upper_bound(timestamps_, timestamps_ + count, end);
    ...
    void OrderBookMmapFile::setup_column_pointers() {
        const auto count = static_cast<size_t>(header_->book_count);
        size_t data_bytes = 0;
        ...
        constexpr size_t price_cols = kLevels * 2;
        constexpr size_t qty_cols = kLevels * 2;
        constexpr size_t order_cols = kLevels * 2;
        ...
        if (size_t end_offset = 0; !checked_add(static_cast<size_t>(header_->data_offset), data_bytes, end_offset) ||
    ...
    Result<void> OrderBookMmapWriter::write_books(const std::string& path,
        ...
        std::ranges::sort(books, [](const OrderBook& a, const OrderBook& b) {
        ...
        if (!out) {
            return Result<void>(Error(Error::Code::IoError, "Unable to open order book mmap output file"));
        }
    ...
    Result<void> OrderBookMmapWriter::validate_books(const std::vector<OrderBook>& books) const {
        ...
        for (size_t i = 0; i < books.size(); ++i) {
            ...
            if (book.timestamp.microseconds() <= 0) {
                return Result<void>(Error(Error::Code::InvalidArgument, "Book timestamp must be positive"));
            }
            if (i > 0 && book.timestamp < last) {
                return Result<void>(Error(Error::Code::InvalidArgument, "Books must be sorted by timestamp"));
            }
            last = book.timestamp;
        }
        return Ok();
