...
namespace regimeflow::data
{
    ...
    void DatabaseDataSource::set_corporate_actions(const SymbolId symbol, std::vector<CorporateAction> actions) {
    ...
    std::vector<SymbolInfo> DatabaseDataSource::get_available_symbols() const {
        ...
        if (!config_.symbols_table.empty()) {
            const auto db_meta = client_->list_symbols_with_metadata(config_.symbols_table);
        ...
        for (const auto& info : symbols) {
            for (auto aliases = adjuster_.aliases_for(info.id); auto alias : aliases) {
    ...
    TimeRange DatabaseDataSource::get_available_range(const SymbolId symbol) const {
        if (!client_) {
            throw std::runtime_error("Database client not configured");
        }
    ...
    std::vector<Bar> DatabaseDataSource::get_bars(const SymbolId symbol, const TimeRange range, const BarType bar_type) {
        if (!client_) {
            throw std::runtime_error("Database client not configured");
        }
    ...
    std::vector<Tick> DatabaseDataSource::get_ticks(const SymbolId symbol, const TimeRange range) {
        if (!client_) {
            throw std::runtime_error("Database client not configured");
        }
        const auto resolved = adjuster_.resolve_symbol(symbol, range.start);
    ...
    std::vector<OrderBook> DatabaseDataSource::get_order_books(const SymbolId symbol, const TimeRange range) {
        if (!client_) {
            throw std::runtime_error("Database client not configured");
        }
        const auto resolved = adjuster_.resolve_symbol(symbol, range.start);
    ...
    std::vector<CorporateAction> DatabaseDataSource::get_corporate_actions(const SymbolId symbol,
