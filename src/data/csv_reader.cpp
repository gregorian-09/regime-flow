...
namespace regimeflow::data
{
    namespace {
        ...
        bool within_trading_hours(const Timestamp& ts, const int start_seconds, const int end_seconds) {
            auto seconds = static_cast<std::time_t>(ts.microseconds() / 1'000'000);
            std::tm tm = {};
#if defined(_WIN32)
            gmtime_s(&tm, &seconds);
#else
            gmtime_r(&seconds, &tm);
#endif
            const int secs = tm.tm_hour * 3600 + tm.tm_min * 60 + tm.tm_sec;
        ...
        void ensure_required_columns(const std::map<std::string, int>& mapping) {
            for (const char* required[] = {"timestamp", "open", "high", "low", "close", "volume"}; const auto* name : required) {
        ...
        [[maybe_unused]] std::string resolve_action_path(const CSVDataSource::Config& config,
            ...
            if (auto pos = filename.find("{symbol}"); pos != std::string::npos) {
            ...
            if (std::ifstream fallback_probe(fallback); fallback_probe) {
                return fallback.string();
            }
            if (std::filesystem::path dir_path(directory); std::filesystem::exists(dir_path) && std::filesystem::is_directory(dir_path)) {
        ...
        std::map<std::string, int> resolve_action_mapping(const std::string& header,
            ...
            const auto aliases = action_aliases();
            const auto fields = split_line(header, delimiter);
            for (size_t i = 0; i < fields.size(); ++i) {
                const auto normalized = normalize_header(fields[i]);
        ...
        std::vector<CorporateAction> parse_actions_file(const CSVDataSource::Config& config,
            ...
            int type_col = col.contains("action_type") ? col["action_type"] : 0;
            int date_col = col.contains("effective_date") ? col["effective_date"] : 1;
            int factor_col = col.contains("factor") ? col["factor"] : -1;
            int amount_col = col.contains("amount") ? col["amount"] : -1;
            int new_symbol_col = col.contains("new_symbol") ? col["new_symbol"] : -1;
    ...
    std::vector<SymbolInfo> CSVDataSource::get_available_symbols() const {
        ...
        for (const auto& id : symbol_to_path_ | std::views::keys) {
    ...
    TimeRange CSVDataSource::get_available_range(const SymbolId symbol) const {
        TimeRange range;
        const std::string path = resolve_path(symbol);
        if (path.empty()) {
            return range;
        }
        const auto resolved = adjuster_.resolve_symbol(symbol);
        if (const auto bars = parse_bars(resolved, path, range, BarType::Time_1Day); !bars.empty()) {
    ...
    std::vector<Bar> CSVDataSource::get_bars(const SymbolId symbol, const TimeRange range, const BarType bar_type) {
        const std::string path = resolve_path(symbol);
        if (path.empty()) {
            return {};
        }
        const auto resolved = adjuster_.resolve_symbol(symbol, range.start);
    ...
    std::unique_ptr<DataIterator> CSVDataSource::create_iterator(
        const std::vector<SymbolId>& symbols, const TimeRange range, const BarType bar_type) {
    ...
    std::vector<CorporateAction> CSVDataSource::get_corporate_actions(SymbolId symbol, const TimeRange range) {
        symbol = adjuster_.resolve_symbol(symbol);
        ensure_actions_loaded(symbol);
        const auto cached = action_cache_.find(symbol);
    ...
    void CSVDataSource::set_corporate_actions(const SymbolId symbol, std::vector<CorporateAction> actions) const
    {
        adjuster_.add_actions(symbol, std::move(actions));
    }

    void CSVDataSource::ensure_actions_loaded(SymbolId symbol) const {
        if (action_cache_.contains(symbol)) {
            return;
        }
        std::vector<CorporateAction> actions;
        const std::string directory = config_.actions_directory.empty()
            ? config_.data_directory
            : config_.actions_directory;
        if (!directory.empty()) {
            const auto symbol_str = SymbolRegistry::instance().lookup(symbol);
            std::string filename = config_.actions_file_pattern;
            if (const auto pos = filename.find("{symbol}"); pos != std::string::npos) {
                filename.replace(pos, 8, symbol_str);
            }
            const std::filesystem::path resolved = std::filesystem::path(directory) / filename;
            actions = parse_actions_file(config_, symbol, resolved.string());
            if (actions.empty()) {
                const std::filesystem::path fallback = std::filesystem::path(directory)
                    / (symbol_str + "_actions.csv");
    ...
    std::string CSVDataSource::resolve_path(SymbolId symbol) const {
        const SymbolId resolved = adjuster_.resolve_symbol(symbol);
        symbol = resolved;
        if (const auto it = symbol_to_path_.find(symbol); it != symbol_to_path_.end()) {
            return it->second;
        }
        const auto symbol_str = SymbolRegistry::instance().lookup(symbol);
        ...
        const std::filesystem::path path = std::filesystem::path(config_.data_directory) / filename;
    ...
    void CSVDataSource::scan_directory() {
        ...
        const std::filesystem::path root(config_.data_directory);
        if (!std::filesystem::exists(root) || !std::filesystem::is_directory(root)) {
            return;
        }

        const std::string pattern = config_.file_pattern;
        const auto marker = pattern.find("{symbol}");
        const std::string prefix = marker == std::string::npos ? "" : pattern.substr(0, marker);
        const std::string suffix = marker == std::string::npos ? "" : pattern.substr(marker + 8);
    ...
    std::map<std::string, int> CSVDataSource::resolve_mapping(const std::string& header) const {
        std::map<std::string, int> mapping;
        const auto aliases = config_.column_aliases.empty() ? default_aliases() : config_.column_aliases;
        const auto fields = split_line(header, config_.delimiter);
        for (size_t i = 0; i < fields.size(); ++i) {
            const auto normalized = normalize_header(fields[i]);
            auto canonical = apply_alias(normalized, aliases);
            mapping[canonical] = static_cast<int>(i);
    ...
    std::vector<Bar> CSVDataSource::parse_bars(SymbolId symbol, const std::string& path,
        ...
        int ts_col = col.contains("timestamp") ? col["timestamp"] : 0;
        int open_col = col.contains("open") ? col["open"] : 1;
        int high_col = col.contains("high") ? col["high"] : 2;
        int low_col = col.contains("low") ? col["low"] : 3;
        int close_col = col.contains("close") ? col["close"] : 4;
        int vol_col = col.contains("volume") ? col["volume"] : 5;
        int symbol_col = col.contains(config_.symbol_column) ? col[config_.symbol_column] : -1;
        ...
        auto handle_issue = [&](const ValidationSeverity severity,
        ...
        while (std::getline(file, line)) {
            ...
            if (config_.validation.check_gap && has_last_ts) {
                if (Duration gap = bar.timestamp - last_ts; gap.total_microseconds() > config_.validation.max_gap.total_microseconds()) {
            ...
            if (config_.validation.check_price_jump && has_last_close && last_close != 0.0) {
                if (double jump = std::abs(bar.close - last_close) / std::abs(last_close); jump > config_.validation.max_jump_pct) {
            ...
            if (config_.validation.check_outliers &&
                ...
                if (double stddev = price_stats.stddev(); stddev > 0.0) {
            ...
            if (config_.validation.check_outliers &&
                ...
                if (double stddev = volume_stats.stddev(); stddev > 0.0) {
        ...
        if (config_.fill_missing_bars || fill_on_gap) {
            if (auto interval = bar_interval_for(bar_type); interval.has_value()) {
