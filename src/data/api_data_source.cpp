...
namespace regimeflow::data
{
    namespace {
        ...
        std::map<std::string, int> map_headers(const std::string& header_line) {
            ...
            const auto aliases = default_aliases();
            const auto fields = split_line(header_line, ',');
        ...
        Timestamp parse_timestamp(const std::string& value) {
            const bool numeric = !value.empty() && std::ranges::all_of(value, [](unsigned char c) {
                return std::isdigit(c);
            });
            if (numeric) {
                const auto num = std::stoll(value);
                if (value.size() > 13) {
        ...
        std::vector<Bar> parse_csv_bars(const std::string& data, SymbolId symbol) {
            ...
            if (!mapping.contains("timestamp") || !mapping.contains("open") || !mapping.contains("high") ||
                !mapping.contains("low") || !mapping.contains("close") || !mapping.contains("volume")) {
                return out;
                }
        ...
        std::vector<Tick> parse_csv_ticks(const std::string& data, SymbolId symbol) {
            ...
            auto mapping = map_headers(line);
            if (!mapping.contains("timestamp") || !mapping.contains("price") || !mapping.contains("quantity")) {
                return out;
            }
            while (std::getline(stream, line)) {
                if (line.empty()) {
        ...
        Result<std::string> http_get(const std::string& url,
            ...
            if (!curl) {
                return Result<std::string>(Error(Error::Code::NetworkError, "Failed to init curl"));
            }
            std::string response;
    ...
    std::vector<Bar> ApiDataSource::get_bars(const SymbolId symbol, const TimeRange range, const BarType bar_type) {
        const auto& symbol_name = SymbolRegistry::instance().lookup(symbol);
        if (symbol_name.empty()) {
            return {};
        }
        const std::string url = build_url(config_.bars_endpoint, symbol_name, range, bar_type);
    ...
    std::vector<Tick> ApiDataSource::get_ticks(SymbolId symbol, TimeRange range) {
        ...
        const std::string url = build_url(config_.ticks_endpoint, symbol_name, range, BarType::Tick);
        std::vector<std::string> headers;
        if (!config_.api_key.empty()) {
            headers.push_back(config_.api_key_header + ": " + config_.api_key);
        }
