...
namespace regimeflow::data
{
...
#ifdef REGIMEFLOW_USE_CURL
        size_t curl_write_cb(const void* contents, const size_t size, const size_t nmemb, void* userp) {
    ...
    Result<std::string> AlpacaDataClient::get_bars(const std::vector<std::string>& symbols,
        ...
        if (symbols.empty()) {
            return Result<std::string>(Error(Error::Code::InvalidArgument, "No symbols provided"));
        }
        if (timeframe.empty()) {
            return Result<std::string>(Error(Error::Code::InvalidArgument, "Missing timeframe"));
        }
        std::ostringstream query;
    ...
    Result<std::string> AlpacaDataClient::get_snapshot(const std::string& symbol) const {
        if (symbol.empty()) {
            return Result<std::string>(Error(Error::Code::InvalidArgument, "Missing symbol"));
        }
        const std::string path = "/v2/stocks/" + symbol + "/snapshot";
    ...
    Result<std::string> AlpacaDataClient::rest_get(const std::string& base_url,
        ...
        if (config_.api_key.empty() || config_.secret_key.empty()) {
            return Result<std::string>(Error(Error::Code::ConfigError, "Alpaca API keys not configured"));
        }
        if (base_url.empty()) {
            return Result<std::string>(Error(Error::Code::ConfigError, "Missing base URL"));
        }
        ...
        const std::string key_header = "APCA-API-KEY-ID: " + config_.api_key;
        const std::string secret_header = "APCA-API-SECRET-KEY: " + config_.secret_key;
