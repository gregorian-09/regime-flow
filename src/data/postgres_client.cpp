...
{
    ConnectionPool::ConnectionPool(std::string connection_string, const int size)
    ...
    std::vector<Bar> PostgresDbClient::query_bars(const SymbolId symbol, const TimeRange range, BarType bar_type) {
        std::vector<Bar> out;
#ifdef REGIMEFLOW_USE_LIBPQ
    ...
    std::vector<Tick> PostgresDbClient::query_ticks(const SymbolId symbol, const TimeRange range) {
        std::vector<Tick> out;
#ifdef REGIMEFLOW_USE_LIBPQ
        const auto& symbol_name = SymbolRegistry::instance().lookup(symbol);
        if (symbol_name.empty()) {
    ...
    TimeRange PostgresDbClient::get_available_range(const SymbolId symbol) {
        TimeRange range;
#ifdef REGIMEFLOW_USE_LIBPQ
        const auto& symbol_name = SymbolRegistry::instance().lookup(symbol);
        if (symbol_name.empty()) {
    ...
    std::vector<CorporateAction> PostgresDbClient::query_corporate_actions(const SymbolId symbol,
    ...
    std::vector<OrderBook> PostgresDbClient::query_order_books(const SymbolId symbol, const TimeRange range) {
    ...
    Result<void> PostgresDbClient::execute_query(const std::string& sql,
        ...
        (void)params;
        (void)row_handler;
        return Result<void>(Error(Error::Code::IoError, "Postgres support not enabled"));
#endif
    }
}  // namespace regimeflow::data
