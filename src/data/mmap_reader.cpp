...
namespace regimeflow::data
{
    ...
    std::pair<size_t, size_t> MemoryMappedDataFile::find_range(const TimeRange range) const {
    ...
    void MemoryMappedDataFile::setup_column_pointers() {
        const auto count = static_cast<size_t>(header_->bar_count);
        ...
        if (size_t end_offset = 0; !checked_add(static_cast<size_t>(header_->data_offset), total_bytes, end_offset) ||
        ...
        if (header_->index_offset > 0 && header_->index_offset < file_size_) {
            ...
            const size_t index_bytes = file_size_ - static_cast<size_t>(header_->index_offset);
