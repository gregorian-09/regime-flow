...
namespace regimeflow::data
{
    ...
    TimeRange MetadataOverlayDataSource::get_available_range(const SymbolId symbol) const {
        return inner_->get_available_range(symbol);
    }

    std::vector<Bar> MetadataOverlayDataSource::get_bars(const SymbolId symbol, const TimeRange range,
                                                         const BarType bar_type) {
        return inner_->get_bars(symbol, range, bar_type);
    }

    std::vector<Tick> MetadataOverlayDataSource::get_ticks(const SymbolId symbol, const TimeRange range) {
        return inner_->get_ticks(symbol, range);
    }

    std::vector<OrderBook> MetadataOverlayDataSource::get_order_books(const SymbolId symbol,
    ...
    std::vector<CorporateAction> MetadataOverlayDataSource::get_corporate_actions(const SymbolId symbol,
                                                                                  const TimeRange range) {
