...
namespace regimeflow::data
{
    namespace {
        ...
        std::string bar_timeframe(const BarType bar_type) {
            switch (bar_type) {
        ...
        std::string get_next_page_token(const common::JsonValue::Object& root) {
            const auto it = root.find("next_page_token");
            if (it == root.end()) {
                return {};
            }
            if (const auto* str = it->second.as_string()) {
        ...
        TradesPage parse_trades_page_json(const std::string& payload) {
            ...
            out.next_page_token = get_next_page_token(*root);
            const auto it = root->find("trades");
            if (it == root->end()) {
                return out;
            }
    ...
    std::vector<SymbolInfo> AlpacaDataSource::get_available_symbols() const {
        ...
        if (symbols_.empty()) {
            ...
            for (const auto& info : assets | std::views::values) {
                out.push_back(info);
            }
            return out;
        }
        out.reserve(symbols_.size());
        for (const auto& symbol : symbols_) {
            if (auto it = assets.find(symbol); it != assets.end()) {
    ...
    std::vector<Bar> AlpacaDataSource::get_bars(SymbolId symbol, TimeRange range,
                                                BarType bar_type) {
        if (!allowed_symbols_.empty() && !allowed_symbols_.contains(symbol)) {
            return {};
        }
        ...
        for (int page = 0; page < kMaxPages; ++page) {
            ...
            auto [bars, next_page_token] = parse_bars_page_json(res.value());
            merge_bars(out, std::move(bars));
            if (next_page_token.empty() || next_page_token == last_token) {
                break;
            }
    ...
    std::vector<Tick> AlpacaDataSource::get_ticks(SymbolId symbol, TimeRange range) {
        ...
        for (int page = 0; page < kMaxPages; ++page) {
            ...
            auto [ticks, next_page_token] = parse_trades_page_json(res.value());
            merge_ticks(out, std::move(ticks));
            if (next_page_token.empty() || next_page_token == last_token) {
                break;
            }
            last_token = next_page_token;
