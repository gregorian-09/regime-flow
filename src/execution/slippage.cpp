...
{
    Price ZeroSlippageModel::execution_price(const engine::Order&, const Price reference_price) const {
        return reference_price;
    }

    FixedBpsSlippageModel::FixedBpsSlippageModel(const double bps) : bps_(bps) {}

    Price FixedBpsSlippageModel::execution_price(const engine::Order& order, const Price reference_price) const {
    ...
    RegimeBpsSlippageModel::RegimeBpsSlippageModel(
        const double default_bps, std::unordered_map<regime::RegimeType, double> bps_map)
    ...
    Price RegimeBpsSlippageModel::execution_price(const engine::Order& order,
                                                  const Price reference_price) const {
        double bps = default_bps_;
        if (const auto it = order.metadata.find("regime"); it != order.metadata.end()) {
            if (const auto parsed = parse_regime(it->second)) {
                if (const auto map_it = bps_map_.find(*parsed); map_it != bps_map_.end()) {
                    bps = map_it->second;
                }
            }
        }
