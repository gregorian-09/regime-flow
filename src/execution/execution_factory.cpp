...
namespace regimeflow::execution
{
    ...
    std::unique_ptr<ExecutionModel> ExecutionFactory::create_execution_model(const Config& config) {
        const auto type = config.get_as<std::string>("model")
        ...
        const auto plugin_cfg = plugin_params_or_full(config);
    ...
    std::unique_ptr<SlippageModel> ExecutionFactory::create_slippage_model(const Config& config) {
        const auto type = config.get_as<std::string>("slippage.type").value_or("zero");
        if (type == "zero") {
        ...
        const auto model = config.get_as<std::string>("model")
            .value_or(config.get_as<std::string>("type").value_or(""));
        if (const auto plugin_name = !model.empty() ? model : type; !plugin_name.empty()) {
            const auto plugin_cfg = plugin_params_or_full(config);
            const auto plugin = plugins::PluginRegistry::instance()
    ...
    std::unique_ptr<CommissionModel> ExecutionFactory::create_commission_model(const Config& config) {
        const auto type = config.get_as<std::string>("commission.type").value_or("zero");
        if (type == "zero") {
    ...
    std::unique_ptr<TransactionCostModel> ExecutionFactory::create_transaction_cost_model(
        const Config& config) {
        const auto type = config.get_as<std::string>("transaction_cost.type").value_or("zero");
        if (type == "zero") {
    ...
    std::unique_ptr<MarketImpactModel> ExecutionFactory::create_market_impact_model(
        const Config& config) {
        const auto type = config.get_as<std::string>("market_impact.type").value_or("zero");
        if (type == "zero") {
    ...
    std::unique_ptr<LatencyModel> ExecutionFactory::create_latency_model(const Config& config) {
        const auto ms = config.get_as<int64_t>("latency.ms").value_or(0);
