...
namespace regimeflow::metrics
{
    ...
    double PerformanceCalculator::compute_periods_per_year(
        ...
        const double avg_delta = total_seconds / static_cast<double>(curve.size() - 1);
        if (avg_delta <= 0.0) {
            return 252.0;
        }
    ...
    std::vector<double> PerformanceCalculator::compute_returns(
        ...
        for (size_t i = 1; i < curve.size(); ++i) {
            if (const double prev = curve[i - 1].equity; prev == 0.0) {
    ...
    double PerformanceCalculator::max_drawdown(
        ...
        for (const auto& snap : curve) {
            ...
            if (const double dd = peak > 0.0 ? (peak - snap.equity) / peak : 0.0; dd > max_dd) {
                max_dd = dd;
                start = peak_time;
                end = snap.timestamp;
            }
    ...
    PerformanceCalculator::build_trades_from_fills(const std::vector<engine::Fill>& fills) const {
        ...
        for (const auto& fill : fills) {
            ...
            while (!lots.empty() && (remaining * lots.front().quantity) < 0.0) {
                ...
                lots.pop_front();
                const double close_qty = std::min(std::abs(remaining), std::abs(quantity));
                ...
                const double pnl = close_qty * (fill.price - price) * sign - open_commission
                    - close_commission;
                const double notional = close_qty * price;
                const double duration_days = static_cast<double>(
                    (fill.timestamp - timestamp).total_seconds()) / (24.0 * 3600.0);
                trades.push_back({pnl, notional, duration_days});
                const double new_qty = quantity + close_qty * (quantity > 0 ? -1.0 : 1.0);
                const double new_commission = commission - open_commission;
                if (new_qty != 0.0) {
                    lots.push_front({new_qty, price, timestamp, new_commission});
                }
        ...
        const std::vector<double>* benchmark_returns) const
    {
        ...
        double years = static_cast<double>((equity_curve.back().timestamp - equity_curve.front().timestamp).total_seconds())
            / (365.25 * 24.0 * 3600.0);
        if (years > 0) {
        ...
        if (!returns.empty()) {
            ...
            if (double p05 = percentile(returns, 0.05); p05 != 0.0) {
        ...
        if (benchmark_returns && benchmark_returns->size() == returns.size()) {
            ...
            if (double tracking_error = stddev(active, active_mean); tracking_error > 0.0) {
            ...
            if (double beta = var > 0.0 ? cov / var : 0.0; beta != 0.0) {
        ...
        if (!equity_curve.empty()) {
            ...
            for (const auto & it : std::ranges::reverse_view(equity_curve)) {
                open_positions = 0;
                unrealized = 0.0;
                for (const auto& pos : it.positions | std::views::values) {
                    if (pos.quantity != 0.0) {
                        open_positions += 1;
    ...
    AttributionResult PerformanceCalculator::calculate_attribution(
        const std::vector<engine::PortfolioSnapshot>& equity_curve,
        const std::vector<regime::RegimeState>& regimes,
        const std::map<std::string, std::vector<double>>& factor_returns) const
    {
        AttributionResult result;
        const auto returns = compute_returns(equity_curve);
        if (returns.empty()) {
            return result;
        }
        if (!regimes.empty()) {
        ...
        const double avg_return = mean(returns);
        ...
        for (const auto& [name, factor] : factor_returns) {
            ...
            const double beta = var > 0.0 ? cov / var : 0.0;
            const double contribution = beta * factor_mean;
    ...
    double PerformanceCalculator::regime_robustness_score(
        ...
        for (const auto& metrics : regime_metrics | std::views::values) {
            sharpes.push_back(metrics.summary.sharpe_ratio);
        }
        if (sharpes.empty()) {
            return 0.0;
        }
        const double avg = mean(sharpes);
        const double dev = stddev(sharpes, avg);
        if (avg == 0.0) {
            return 0.0;
        }
        return 1.0 - (dev / avg);
