...
{
    void MetricsTracker::update(const Timestamp timestamp, const double equity) {
        last_equity_ = equity;
        equity_curve_.add_point(timestamp, equity);
        drawdown_.update(timestamp, equity);
    }

    void MetricsTracker::update(const Timestamp timestamp, const engine::Portfolio& portfolio,
