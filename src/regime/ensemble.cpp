...
{
    void EnsembleRegimeDetector::add_detector(std::unique_ptr<RegimeDetector> detector, const double weight) {
    ...
    void EnsembleRegimeDetector::configure(const Config& config) {
        if (const auto voting = config.get_as<std::string>("ensemble.voting_method")) {
            if (*voting == "majority") {
        ...
        if (const auto detectors = config.get_as<ConfigValue::Array>("ensemble.detectors")) {
            detectors_.clear();
            weights_.clear();
            for (const auto& entry : *detectors) {
                ...
                const double weight = det_cfg.get_as<double>("weight").value_or(1.0);
                if (det) {
    ...
    RegimeState EnsembleRegimeDetector::combine(const std::vector<RegimeState>& states,
        ...
        constexpr size_t prob_size = result.probabilities.size();

        auto normalize_probs = [&](std::vector<double>& probs) {
            const double sum = std::accumulate(probs.begin(), probs.end(), 0.0);
            if (sum <= 0.0) {
                const double uniform = probs.empty() ? 0.0 : 1.0 / static_cast<double>(probs.size());
                std::ranges::fill(probs, uniform);
                return;
            }
            for (double& p : probs) {
                p /= sum;
        ...
        if (voting_method_ == VotingMethod::Majority) {
            ...
            std::ranges::copy(probs, result.probabilities.begin());
        } else if (voting_method_ == VotingMethod::ConfidenceWeighted) {
            for (size_t i = 0; i < states.size(); ++i) {
                const double w = i < weights_.size() ? weights_[i] : 1.0;
                const double conf = states[i].confidence > 0 ? states[i].confidence : 1.0;
            ...
            double maxv = *std::ranges::max_element(logp);
            double sum = 0.0;
            for (double& v : logp) {
                v = std::exp(v - maxv);
                sum += v;
