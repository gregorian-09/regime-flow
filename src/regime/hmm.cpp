...
namespace regimeflow::regime
{
    namespace {
        ...
        double log_sum_exp(const std::vector<double>& values) {
            ...
            const double maxv = *std::ranges::max_element(values);
            double sum = 0.0;
        ...
        std::vector<double> softmax(const std::vector<double>& logp) {
            const double maxv = *std::ranges::max_element(logp);
            ...
            if (sum == 0) {
                return std::vector<double>(logp.size(), 1.0 / static_cast<double>(logp.size()));
            }
            for (double& v : exps) {
                v /= sum;
            }
    ...
    double HMMRegimeDetector::log_likelihood(const std::vector<FeatureVector>& data) const {
        ...
        const auto log_alpha = forward_log(data);
    ...
    void HMMRegimeDetector::set_normalize_features(const bool normalize) {
        extractor_.set_normalize(normalize);
    }

    void HMMRegimeDetector::set_normalization_mode(const NormalizationMode mode) {
    ...
    void HMMRegimeDetector::save(const std::string& path) const {
        ...
        out << "\n";
        out << "initial ";
        for (const double v : initial_) out << v << " ";
        out << "\n";
        out << "transition\n";
        for (int i = 0; i < states_; ++i) {
            for (int j = 0; j < states_; ++j) {
        ...
        for (const auto& [mean, variance] : emissions_) {
            out << "mean ";
            for (const double v : mean) out << v << " ";
            out << "\n";
            out << "variance ";
            for (const double v : variance) out << v << " ";
            out << "\n";
        }
    }
    ...
    void HMMRegimeDetector::configure(const Config& config) {
        const int states = static_cast<int>(config.get_as<int64_t>("hmm.states").value_or(states_));
        const int window = static_cast<int>(config.get_as<int64_t>("hmm.window").value_or(window_));
        if (states != states_) {
            states_ = states;
            transition_.assign(states_, std::vector<double>(states_, 1.0 / states_));
            probabilities_.assign(states_, 1.0 / states_);
        ...
        if (const auto normalize = config.get_as<bool>("hmm.normalize_features")) {
            extractor_.set_normalize(*normalize);
        }
        if (const auto norm_mode = config.get_as<std::string>("hmm.normalization")) {
            if (*norm_mode == "zscore") {
        ...
        if (const auto v = config.get_as<bool>("hmm.kalman_enabled")) {
            enable_kalman_ = *v;
        }
        if (const auto v = config.get_as<double>("hmm.kalman_process_noise")) {
            kalman_process_noise_ = *v;
        }
        if (const auto v = config.get_as<double>("hmm.kalman_measurement_noise")) {
