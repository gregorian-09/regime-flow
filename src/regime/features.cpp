...
{
    FeatureExtractor::FeatureExtractor(const int window) : window_(window) {}

    void FeatureExtractor::set_window(const int window) {
        window_ = window;
        returns_.clear();
        volumes_.clear();
        ranges_.clear();
    ...
    void FeatureExtractor::set_normalize(const bool normalize) {
        normalization_mode_ = normalize ? NormalizationMode::ZScore : NormalizationMode::None;
    }

    void FeatureExtractor::set_normalization_mode(const NormalizationMode mode) {
        normalization_mode_ = mode;
    }

    void FeatureExtractor::update_cross_asset_features(const double market_breadth,
                                                       const double sector_rotation,
                                                       const double correlation_eigen,
                                                       const double risk_appetite) {
    ...
    FeatureVector FeatureExtractor::on_bar(const data::Bar& bar) {
        ...
        const double current_vol = compute_volatility();
    ...
    FeatureVector FeatureExtractor::build_features(const double ret, const double log_ret, const double range,
                                                   const double volume, const double current_vol,
                                                   const double spread, const double imbalance) {
        if (features_.empty()) {
        ...
        for (const auto feature : features_) {
            ...
            case FeatureType::UpDownVolumeRatio: {
                    double up = 0.0;
                    double total = 0.0;
                    for (const double v : signed_volumes_) {
                        if (v > 0) up += v;
                        total += std::abs(v);
                    }
    ...
    double FeatureExtractor::compute_zscore(const std::deque<double>& series, const double value) const {
        if (series.size() < 2) {
            return 0.0;
        }
        double mean = 0.0;
    ...
    double FeatureExtractor::normalize_value(const FeatureType feature, const double value) {
    ...
    double FeatureExtractor::compute_percentile(std::vector<double> values, const double percentile) const {
        if (values.empty()) {
            return 0.0;
        }
        if (percentile <= 0.0) {
