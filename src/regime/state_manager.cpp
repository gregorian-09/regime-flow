...
{
    RegimeStateManager::RegimeStateManager(const size_t history_size) : history_size_(history_size) {}
    ...
    std::vector<RegimeTransition> RegimeStateManager::recent_transitions(const size_t n) const {
        std::vector<RegimeTransition> out;
        if (n == 0 || transition_history_.empty()) {
            return out;
        }
        const size_t start = transition_history_.size() > n ? transition_history_.size() - n : 0;
    ...
    double RegimeStateManager::avg_regime_duration(const RegimeType regime) const {
        double total = 0.0;
        int count = 0;
        for (const auto& transition : transition_history_) {
    ...
    std::vector<std::vector<double>> RegimeStateManager::empirical_transition_matrix() const {
        ...
        for (const auto& transition : transition_history_) {
            const size_t from = to_index(transition.from);
            const size_t to = to_index(transition.to);
    ...
    size_t RegimeStateManager::to_index(const RegimeType regime) const {
        switch (regime) {
