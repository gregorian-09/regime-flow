...
{
    CustomFeatureBuilder::CustomFeatureBuilder(const int window) : window_(window) {}

    regimeflow::regime::FeatureVector CustomFeatureBuilder::on_bar(
        const regimeflow::data::Bar& bar) {
        if (last_close_ > 0.0) {
            const double ret = (bar.close - last_close_) / last_close_;
    ...
    double CustomFeatureBuilder::compute_volatility() const {
        ...
        double mean = 0.0;
        for (const double r : returns_) mean += r;
    ...
    double CustomFeatureBuilder::compute_skew() const {
        ...
        const double variance = m2 / static_cast<double>(returns_.size());
    ...
    regimeflow::regime::RegimeState CustomRegimeDetector::on_bar(
        const regimeflow::data::Bar& bar) {
        const auto features = features_.on_bar(bar);
    ...
    void CustomRegimeDetector::configure(const regimeflow::Config& config) {
        if (const auto v = config.get_as<int64_t>("window")) {
            features_ = CustomFeatureBuilder(static_cast<int>(*v));
        }
        if (const auto v = config.get_as<double>("trend_threshold")) {
            trend_threshold_ = *v;
        }
        if (const auto v = config.get_as<double>("vol_threshold")) {
            vol_threshold_ = *v;
        }
    }
