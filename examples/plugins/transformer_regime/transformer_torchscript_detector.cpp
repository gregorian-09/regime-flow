...
namespace transformer_regime
{
    namespace {
        ...
        regimeflow::regime::RegimeType idx_to_regime(const int idx) {
            switch (idx) {
    ...
    void TorchscriptRegimeDetector::configure(const regimeflow::Config& config) {
        if (const auto v = config.get_as<std::string>("model_path")) {
            model_path_ = *v;
        }
        if (const auto v = config.get_as<int64_t>("feature_window")) {
            window_ = std::max<int64_t>(30, *v);
        }
        if (const auto v = config.get_as<int64_t>("feature_dim")) {
