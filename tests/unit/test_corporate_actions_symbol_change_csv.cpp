...
namespace
{
    TEST(CsvDataSourceTest, SymbolChangeAliasesAvailableSymbolsAndRange) {
        ...
        const regimeflow::data::CSVDataSource source(cfg);
        const auto old_symbol = regimeflow::SymbolRegistry::instance().intern("TEST");
        ...
        const auto symbols = source.get_available_symbols();
        bool has_new = false;
        ...
        const auto new_symbol = regimeflow::SymbolRegistry::instance().intern("TEST2");
        auto [start, end] = source.get_available_range(new_symbol);
        EXPECT_GT(start.microseconds(), 0);
        EXPECT_GT(end.microseconds(), 0);
    }
}  // namespace
