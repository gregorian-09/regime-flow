...
namespace
{
    TEST(CorporateActionAdjusterTest, AdjustsForSplit) {
        ...
        const auto adjusted = adjuster.adjust_bar(symbol, bar);
    ...
    TEST(CorporateActionAdjusterTest, AdjustsForDividend) {
        regimeflow::data::CorporateActionAdjuster adjuster;
        const auto symbol = regimeflow::SymbolRegistry::instance().intern("DIV");
    ...
    TEST(CorporateActionAdjusterTest, AppliesSymbolChangeFromEffectiveDate) {
        regimeflow::data::CorporateActionAdjuster adjuster;
        const auto old_symbol = regimeflow::SymbolRegistry::instance().intern("OLD");
        ...
        const auto before_adjusted = adjuster.adjust_bar(old_symbol, before);
        ...
        after.timestamp = regimeflow::Timestamp::from_string("2020-01-02 00:00:00",
                                                             "%Y-%m-%d %H:%M:%S");
        const auto after_adjusted = adjuster.adjust_bar(old_symbol, after);
        const auto new_symbol = regimeflow::SymbolRegistry::instance().intern("NEW");
